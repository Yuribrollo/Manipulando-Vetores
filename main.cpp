...
Autor: Yuri de Oliveira Brollo
Data: 2024.10.27

Além das funções originais, o programa é capaz de contar a quantidade de números pares, ìmpares, negativos, positivos, indicar o menor e o maior número da lista e multiplicar todos os valores por um número indicado pelo usuário.
...
void qtdImpPar(int vetor[], int tamanho) { // cria a função, com o vetor e o tamanho da mesma
    int imp = 0, par = 0; // inicia as variaveis imp e par sempre como 0
    for (int i = 0; i < tamanho; i++) {  // cria um for que passa por todos os itens da lista
        if(vetor[i] % 2 != 0){ // verifica se os numeros são impares pelo modulo do item dividido por 2, se for diferente de 0 é ímpar
            imp += 1;  // adiciona 1 na contagem quando a condição for verdadeira
        } else { // se a condição for falsa, ou seja, o módulo da condição anterior for igual a 0, o número é par
            par +=1; // adiciona 1 a contagem de pares
        }
    }

    cout << "Quantidade de Ímpares: " << imp << endl; // saída da quantidade de números ímpares
    cout << "Quantidade de Pares: " << par << endl; // saída da quantidade de números pares

    esperarEnter();  // chama a função para voltar para o menu
}

void qtdNegPos(int vetor[], int tamanho) { // cria a função, com o vetor e o tamanho da mesma
    int neg = 0, pos = 0; // inicia as variaveis neg e pos sempre como 0
    for (int i = 0; i < tamanho; i++) {  // cria um for que passa por todos os itens da lista
        if(vetor[i] < 0){  // se o item da lista for menor que 0, ele é negativo
            neg += 1;   // se a condição for verdadeira, adiciona 1 na contagem de negativos;
        } else { // caso a condição seja falsa, ou seja, maior que 0, o item é positivo
            pos +=1; //adiciona 1 na contagem de numeros positivo
        }
    }

    cout << "Quantidade de Positivos: " << pos << endl; // saída da quantidade de números positivos
    cout << "Quantidade de Negativos: " << neg << endl; // saída da quantidade de números negativos
    esperarEnter();  // chama a função para voltar para o menu
}

void menorMaior(int vetor[], int tamanho) { // cria a função, com o vetor e o tamanho da mesma
    int menor = 0, maior = 0;  // inicia as variaveis menor e maior sempre como 0

    sort(vetor, vetor + tamanho); //organiza todos os itens da lista de ordem crescente 

    menor = vetor[0]; //depois de ordenada, o primeiro item da lista será o menor, então atribui-se à variável "menor" o valor do primeiro item
    maior = vetor[9]; //depois de ordenada, o útimo item da lista será o maior, então atribui-se à variável "maior" o valor do útimo item

    cout << "Menor valor: " << menor << endl; // saída do menor valor
    cout << "Maior valor: " << maior << endl; // saída do maior valor

    esperarEnter();  // chama a função para voltar para o menu
}

void multiplicarLista(int vetor[], int tamanho) { // cria a função, com o vetor e o tamanho da mesma

    int num = 0; //inicia a variavel num sempre como 0

    cout << "Indique o valor que quer multiplicar a lista!"; // pede para o usúario o número pelo qual quer multiplicar a lista
    cin >> num; // atribui a num o valor dado pelo usuário

    for (int i = 0; i < tamanho; i++) { // inicia um for qua passa por todos os itens da lista
        vetor[i] = vetor[i] * num; //multiplica cada item pelo valor indicado pelo usuário
    }
...
int menu() {
    ...
    cout << "8 - Verificar quantidade de ímpares e pares\n"; //indica para o usuário qual número digitar para acessar esse função
    cout << "9 - Verificar quantidade de positivos e negativos\n"; //indica para o usuário qual número digitar para acessar esse função
    cout << "10 - Verificar o menor e o maior valor\n"; //indica para o usuário qual número digitar para acessar esse função
    cout << "11 - Multiplicar lista\n"; //indica para o usuário qual número digitar para acessar esse função
    cout << "0 - Sair\n";
    ...
    do {
        ...
        switch (opcao) {
            ...
            case 7:
                ...
                break;
            case 8:
                qtdImpPar(vetor, TAMANHO); //caso o usúario clicar no númeo correspondente, inicia essa função
            case 9:
                qtdNegPos(vetor, TAMANHO); //caso o usúario clicar no númeo correspondente, inicia essa função
            case 10:
                menorMaior(vetor, TAMANHO); //caso o usúario clicar no númeo correspondente, inicia essa função
            case 11:
                multiplicarLista(vetor, TAMANHO); //caso o usúario clicar no númeo correspondente, inicia essa função
            case 0:
